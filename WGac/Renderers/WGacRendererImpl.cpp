...
public:
    ...
    bool SetInlineObject(vint start, vint length, const InlineObjectProperties& properties) override
    {
        ...
        {
            InlineObject& obj = inlineObjects[i];
            if (start == obj.start && length == obj.length)
            {
                if (obj.properties.callbackId != properties.callbackId) return false;
                if (obj.properties.backgroundImage != properties.backgroundImage) return false;
                reuseIndex = i;
            }
            else
            {
                if (properties.backgroundImage &&
                    obj.properties.backgroundImage == properties.backgroundImage)
                {
                    return false;
                }
        ...
        if (reuseIndex != -1)
        {
            inlineObjects[reuseIndex].properties = properties;
            RebuildLayout();
            return true;
        }
...
// WGacMain - common renderer main function for standard, hosted, and raw modes
void WGacMain(INativeController* nativeController, GuiHostedController* hostedController, bool raw)
{
    ...
    SetWGacResourceManager(nullptr);
}

int SetupWGacRendererInternal(bool hosted, bool raw)
{
    auto nativeController = wayland::GetWGacController();
...
{
    return SetupWGacRendererInternal(false, false);
}

int SetupWGacHostedRenderer()
{
    return SetupWGacRendererInternal(true, false);
}

int SetupRawWGacRenderer()
{
    return SetupWGacRendererInternal(false, true);
}
